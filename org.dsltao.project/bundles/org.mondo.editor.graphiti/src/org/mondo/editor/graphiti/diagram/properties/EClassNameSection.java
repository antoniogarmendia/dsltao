...
/**
 * Section to modify EClass name value.
 * 
 * @author miso partner AnaPescador
 *
 */
public class EClassNameSection extends GFPropertySection implements ITabbedPropertyConstants {
	...
	private void setNewName(TypedEvent e){
		...
		if (pe != null) {
	        ...
	        if (((EClass)bo).getName().compareTo(control.getText())!=0){
	        	if (ModelUtils.existsClassName(getDiagram(), control.getText())){
            		control.setText(((EClass)bo).getName());
					            ...
					            for (Shape shape : cs.getChildren()) {
					                if (shape.getGraphicsAlgorithm() instanceof org.eclipse.graphiti.mm.algorithms.Text) {
					                    ...
					                    if ((text.getValue().compareTo(control.getText())!=0)&& (DiagramUtils.isText(text))){
				                    		...
				                    		List<EReference> refs = DiagramUtils.getEReferencesIncoming(getDiagram(), (EClass)bo);
				                    		...
				                    		((EClass)bo).setName(control.getText());
