...
/**
 * Section to modify EPackage nsprefix value.
 * 
 * @author miso partner AnaPescador
 *
 */
public class EPackageNsPrefixSection extends GFPropertySection implements ITabbedPropertyConstants {
	...
	public void createControls(Composite parent, TabbedPropertySheetPage tabbedPropertySheetPage) {
		...
		nsPrefix = factory.createText(composite, "");
		data = new FormData();
		data.left = new FormAttachment(0, 100);
		data.right = new FormAttachment(50, 0);
		data.top = new FormAttachment(0, VSPACE);
		
		nsPrefix.setLayoutData(data);
		
		nsPrefix.addFocusListener(focusListener);
		nsPrefix.addKeyListener(keyListener);

		
		CLabel valueLabel = factory.createCLabel(composite,  "Ns Prefix:");
		data = new FormData();
		data.left = new FormAttachment(0, 0);
		data.right = new FormAttachment(nsPrefix, -HSPACE);
		data.top = new FormAttachment(nsPrefix, 0, SWT.CENTER);
	...
	private void setNewPrefix(TypedEvent e){
	        	...
	        	if (control.getText().isEmpty()){
			        ...
			        domain.getCommandStack().execute(new RecordingCommand(domain) {
						...
						protected void doExecute() {
							bo.setNsPrefix(control.getText());
						}
			        });
				}
	        }
