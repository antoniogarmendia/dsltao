 ...
 *
 */
public class ExpandInheritanceFeature extends AbstractCustomFeature {
 
    private boolean hasDoneChanges = false;
      
    public ExpandInheritanceFeature(IFeatureProvider fp) {
        super(fp);
    }
 
    @Override
    public String getName() {
        return "Expand Inheritance";
    }
    	...
    	if (bo instanceof EPackage) {
        	for (EClassifier eClassif: ((EPackage)bo).getEClassifiers()){
				if (eClassif instanceof EClass){
					...
					DiagramUtils.expandEClassInheritance((EClass)eClassif,(Diagram)pes[0], getFeatureProvider());
				}
        	}
        	hasDoneChanges = true;
        	DiagramUtils.setCollapseInheritance(getDiagram(), false);
        }    	
    }
