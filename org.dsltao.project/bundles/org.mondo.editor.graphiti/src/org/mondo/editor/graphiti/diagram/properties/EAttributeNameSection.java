...
/**
 * Section to modify EAttribute name value.
 * 
 * @author miso partner AnaPescador
 *
 */
public class EAttributeNameSection extends GFPropertySection implements ITabbedPropertyConstants {
	...
	public void refresh() {
	    ...
	    if (pe != null) {
	        ...
	        if (bo == null)
	            return;
	        String name = ((EAttribute) bo).getName();
	...
	private void setNewName(TypedEvent e){
		...
		if (pe != null) {
	        ...
	        if (((EAttribute)bo).getName().compareTo(control.getText())!=0){
	        	if (ModelUtils.existsAttName(((EAttribute)bo).getEContainingClass(), control.getText())){
            		control.setText(((EAttribute)bo).getName());
							...
							if (pe instanceof ContainerShape) {
					            ...
					            for (Shape shape : cs.getChildren()) {
					                if (shape.getGraphicsAlgorithm() instanceof org.eclipse.graphiti.mm.algorithms.Text) {
				                    	...
				                    	((EAttribute)bo).setName(control.getText());
